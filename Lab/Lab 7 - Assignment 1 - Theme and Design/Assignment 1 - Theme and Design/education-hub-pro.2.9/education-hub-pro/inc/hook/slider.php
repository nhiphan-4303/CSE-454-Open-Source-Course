	...
	function education_hub_get_slider_details( $input ) {
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-post':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = education_hub_get_option( 'featured_slider_post_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
			...
			case 'featured-category':
				...
				$featured_slider_category = education_hub_get_option( 'featured_slider_category' );
			...
			case 'featured-tag':
				...
				$featured_slider_tag = education_hub_get_option( 'featured_slider_tag' );
			...
			case 'featured-image':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++) {
					$photo = education_hub_get_option( 'featured_slider_image_photo_' . $i );
					if ( ! empty( $photo  ) ) {
						$item = array();
						$item['images'][0] = $photo;

						$title      = education_hub_get_option( 'featured_slider_image_title_' . $i );
						$url        = education_hub_get_option( 'featured_slider_image_url_' . $i );
						$caption    = education_hub_get_option( 'featured_slider_image_caption_' . $i );
						$new_window = education_hub_get_option( 'featured_slider_image_new_window_' . $i );
...
if ( ! function_exists( 'education_hub_add_featured_slider' ) ) :
	...
	function education_hub_add_featured_slider() {
		...
		$flag_apply_slider = apply_filters( 'education_hub_filter_slider_status', true );
