...
class EDD_Theme_Updater_Admin {
	...
	/**
	 * Display message in not validated.
	 *
	 * @since 1.0.0
	 */
	function update_nag() {
		...
		if ( isset( $_GET['page'] ) && $this->theme_slug . '-license' === $_GET['page'] ) {
			return;
		}
		$strings = $this->strings;
		$license_status = get_option( $this->theme_slug . '_license_key_status', false );
		$marketplace_url_string = '<a href="' . esc_url( 'https://themepalace.com/my-account/' ). '" target="_blank">' . $strings['theme-palace'] . '</a>';
		?>
			<?php if ( 'valid' !== $license_status ) : ?>
				<div id="message" class="error">
					<p>
					<?php printf( $strings['key-not-activated'], $this->item_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-license' ) ) . '">', '</a>' ); ?>
					<?php printf( $strings['get-license-key'], $marketplace_url_string ); ?>
					</p>
				</div>
			<?php endif ?>
		<?php
